...
function resolveTeamFilter(team: string): string[] | null {
  if (!team || team === "ALL") return null

  // Conference filter
  const confTeams = nflTeams.filter((t) => t.conference === team)
  if (confTeams.length > 0) return confTeams.map((t) => t.abbr)

  // Division filter
  const divTeams = nflTeams.filter((t) => t.division === team)
  if (divTeams.length > 0) return divTeams.map((t) => t.abbr)

  // Direct team abbreviation
  return [team]
}
...
function buildTeamCondition(
  ...
  const teams = resolveTeamFilter(team)
  ...
  params.push(...teams)
  return `${teamColumn} IN (${placeholders})`
}
...
function handleSeasonQuery(
  ...
  const teamCond = buildTeamCondition(opts.team, "r.team", params)
...
function handleWeeklyQuery(
  ...
  const teamCond = buildTeamCondition(opts.team, "w.team", params)
  if (teamCond) conditions.push(teamCond)
